...
; (function ($, window, document, undefined) {
    ...
    var cityObj = function (el, option) {
        ...
        var loadDataHandler = {
            //加载省市区数据
            areaData: function () {
                dataHanlder.splitData(areaData); // 这里调用"数据处理" dataHanlder 对象的 splitData 方法，
            },
            //加载乡镇数据
            townData: function (callback) {
                if (currentArea && currentArea.code) {
                    ...
                    toolHanlder.showLoading();
                    var filePath = './code/' + currentArea.code + '.json';
                    $.ajax({
                        url: filePath, 
                        dataType: 'json',
                        success: function (town) {
                            toolHanlder.hideLoading();
                            // isFunction 判断传入的参数 callback 是否为函数
                        ...
                        error: function (xhr, status, error) {
                            toolHanlder.hideLoading();
                            console.error("加载本地文件失败，错误信息：" + error);
                        }
                    });
                } else {
